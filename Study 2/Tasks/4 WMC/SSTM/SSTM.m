...
function rc=SSTM(subject,sex,year,screenparms,expMessage)
...
try %catch errors for entire function and all subsidiaries
    ...
    if nargin < 2
        screenparms = prepexp;
        subject=getSubject(screenparms,expMessage); % !!
    end
    ...
    NumT.Test=14;       % Number of test trial (Must bigger than 1) !! 30 !!
    ...
    % XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    Text.Fix='+';
    Text.Draw = expMessage(11,:);
    Text.Next = expMessage(12,:);
    
    practicemsg = expMessage(2,:);
    beginmsg = expMessage(3,:);
    taskovermsg = expMessage(6,:);
    ...
    SubID=subject;
    if SubID <= 9
        FileID=['0' int2str(SubID) '-' int2str(sex) '-' int2str(year)];
    else
    ...
    for Trial=1:NumT.Total
        ...
        DisPlayDot(screenparms,Text,Time,Trial,NumIndex,AdjH,AdjV,RecTrial,Dot,Board,w,expMessage);
        ...
        if Trial >= NumT.Pract+1
            ...
            waitfkey(screenparms, expinfo, [expMessage(9,:)])
        end;
    end;

    waitfkey (screenparms, expinfo, taskovermsg);   % ,1
    ...
    RecTask(SubID,subject,sex,year,FileID,IntDate,NumT,RecExp,RecTrial,ShiftAns,Score,Order,RecRT,NumIndex);

    waitfkey (screenparms, expinfo, expMessage(10,:));
