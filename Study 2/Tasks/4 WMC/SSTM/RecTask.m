...
function RecTask(SubID,subject,sex,year,FileID,IntDate,NumT,RecExp,RecTrial,ShiftAns,Score,Order,RecRT,NumIndex)
...
DataFile=fopen(['..\Data\SSTMDetailed\SSTM-' FileID '.txt'],'at');

% if exist('..\Data\SSTM.dat','file')==0
%     SumDataFile=fopen('..\Data\SSTM.dat','at');
if exist(['..\Data\SSTM-' int2str(subject) '-' int2str(sex) '-' int2str(year) '.dat'],'file')==0
    SumDataFile=fopen(['..\Data\SSTM-' int2str(subject) '-' int2str(sex) '-' int2str(year) '.dat'],'at');
