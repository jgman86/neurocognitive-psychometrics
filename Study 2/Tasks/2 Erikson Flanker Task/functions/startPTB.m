...
if ~exist('expinfo.Cursor','var')
    expinfo.Cursor = 0; % Default: Hide Cursor
end

% Set Background color if not already defined
if ~exist('expinfo.Colors.bgColor','var')
    expinfo.Colors.bgColor = [0 0 0]; % Default Background Color is black
end

% Specify if this is a test rund if not provided as input
if ~exist('testExp','var') || isempty(expinfo)
    testExp = 0;
end
...
%% Text Size
expinfo.FontSize = Screen(expinfo.window,'TextSize',40); 
